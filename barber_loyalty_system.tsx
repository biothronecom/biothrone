...
const BarberiaApp = () => {
  ...
  const handleLogin = (role: string, identifier: string) => {
    setUserRole(role);
    if (role === 'CLIENT') {
      setCurrentClientPhone(identifier);
  ...
  const ClientProfile = () => {
    ...
    const [clientTab, setClientTab] = useState<'perfil' | 'citas'>('perfil');
    const scrollToCitas = () => {
          ...
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <div className="flex gap-2">
              <input
                ...
                autoFocus
              />
              <button onClick={searchCustomer} className="bg-blue-600 text-white px-6 py-3 rounded-lg hover:bg-blue-700 flex items-center gap-2 transition-colors">
  ...
  if (appState === 'LANDING') {
    return (
      <div className="animate-in fade-in duration-700">
        <LandingPage onLogin={() => setAppState('LOGIN')} onJoin={() => setAppState('LOGIN')} />
      </div>
    );
  }
        ...
        <div className="app-shell flex flex-col min-h-screen">
          ...
          <main className="flex-1 w-full">
            <div className="container-app py-6 sm:py-8 space-y-6">
              ...
              {currentView === 'client_profile' && userRole === 'CLIENT' && currentClientPhone && <ClientProfile />}
