...
const LoginPage: React.FC<LoginPageProps> = ({ onBack, onLogin }) => {
    const [role, setRole] = useState<'OWNER' | 'BARBER' | 'RECEPTION' | 'CLIENT'>('CLIENT');
                ...
                <div className="bg-white py-8 px-4 shadow-2xl shadow-slate-200 sm:rounded-3xl sm:px-10 border border-slate-100">
                    <div className="flex p-1 bg-slate-100 rounded-2xl mb-8">
                        <button
                            ...
                            className={`flex-1 py-2 text-xs font-bold rounded-xl transition-all ${role === 'CLIENT' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                        >
                            CLIENTE
                        </button>
                        <button
                    ...
                    <form className="space-y-6" onSubmit={handleSubmit}>
                        {role !== 'CLIENT' && (
                            <div>
                                <label htmlFor="role-select" className="block text-sm font-bold text-slate-700 mb-2">
                                    Rol de Trabajo
                                </label>
                                <select
                                    id="role-select"
                        ...
                        <div>
                            ...
                            <div className="relative">
                                <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                    {role === 'CLIENT' ? <Phone className="h-5 w-5 text-slate-400" /> : <User className="h-5 w-5 text-slate-400" />}
                                </div>
                                <input
                                    id="identifier"
                                    name="identifier"
                    ...
                    {role === 'CLIENT' && (
                        <div className="mt-8 text-center">
                            <p className="text-xs text-slate-400 font-medium">
                                Al ingresar, aceptas nuestra política de privacidad y el uso de tus datos para el programa de lealtad.
                            </p>
                        </div>
                    )}
                </div>
